...
mechanistic result should be extrapolated, including ours. The Llama comparison is further complicated by the derivation noted in Section~\ref{sec:limitations}: those three models share a parent, so their differences understate what independent training runs at those scales might show.

The Llama comparison is further complicated by the derivation noted in Section~\ref{sec:limitations}: those three models share a parent, so their differences understate what independent training runs at those scales might show.
